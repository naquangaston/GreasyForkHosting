...
setValue = GM_setValue, getValue = GM_getValue, info_ = GM_info, "quizizz.com" == document.domain && function() {
    ...
    var A = function() {
        ...
        })), console.log("Updated score"), Number(document.getElementsByClassName("score-title")[0].innerText), clearTimeout(T), y && (console.log("Getting answer"), T = setTimeout(l, 3e3))
    };
    let k = "";
    setInterval((() => {
        try {
            let e = i() + "; " + h;
            k != e && (console.clear(), y = 1, v = 0, A()), k = e
        } catch (e) {}
    }));
    setInterval((() => {
        for (let e in E)
