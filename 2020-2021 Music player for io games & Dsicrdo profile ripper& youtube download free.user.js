...
;(function() {
    ...
    function Mute(){abc('Mute (m)').click()}
    function Unmute(){abc('Unmute (m)').click()}
    var didmute=0
    setInterval(e=>{
...
`);
        ...
        if(go){
            ...
            var _=setInterval(()=>{eval("console.dir=function(...c){}");console.dir=function(...c){}
                                   try{
                                       mp42=[...formatSelect.children[0].children]
                                       mp32=[...(formatSelect.children[2]||formatSelect.children[1]).children];
                                       filesize=mp42.filter(e=>e.innerText.includes('(')).map(e=>e.innerText.match(/\w+/g).pop().toLowerCase()=='mb'?e.innerText.match(/[0-9]+\.[0-9]+?/gi)*1:(e.innerText.match(/[0-9]+\.[0-9]+?/gi)*1)/1000)
                                       filesize3=mp32.filter(e=>e.innerText.includes('(')).map(e=>e.innerText.match(/\w+/g).pop().toLowerCase()=='mb'?e.innerText.match(/[0-9]+\.[0-9]+?/gi)*1:(e.innerText.match(/[0-9]+\.[0-9]+?/gi)*1)/1000)
                                       clearInterval(_);
                                       function convert(){
                                           ...
                                           var _2=setInterval(()=>{
                                               ...
                                               setTimeout(()=>{location.hash='Done'},5000)
                                           },1000)
                                       }
                                       if(!discord){
                                           ...
                                           var ind=[];
                                           //
                                           filesize.forEach((e,inx)=>{
                                               if(e<8){ind.push(inx)};
                                           })
                                           window[type+'2'][ind[0]+1].selected=true
                                           convert()
                                       }
