 ...
 */
        ...
        var upgrading = false,loaded=false,AutoUpgrad = true,down = {},DidiU,noads=false,did_=false,loggedkk = false,st=GM_info,playerAlive =true,wasalive = true
...
`));
            ...
            function setBuild(parse) {
                ...
                for(let i in parse.build){
                    try{
                        ...
                        _myWin.document.getElementsByClassName(`${i}_`)[0].innerText=parse.build[i]

                        !(_myWin.document.getElementsByClassName(i)[0].style.width=`${l}%`)
                    }catch(err){}
                }
            }
        ...
        window.addEventListener('keydown', function (e) { const key = e.key; if (down[key]) { return } down[key] = key; /*log('Key down', key, 'Total:', keys(down).length)*/ });
        var info={}
        setInterval(() => {
